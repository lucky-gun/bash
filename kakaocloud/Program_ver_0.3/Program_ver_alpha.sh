...
token="0" # object-storage-token 알게되면 입력  - 초기상태 : 0
...
main_Text(){
	#echo "###### Tech CS Program ver Alpha ##########"
	#echo "######### 버그제보 : lucas.1004 ########"
	#echo -e "\n\n\n사용자의 이름을 입력해주세요."
	#menu_Select
	#echo -e "\n\nDB를 통해 확인중입니다. 잠시만 기다려주세요."
	#db_Check
	echo -e "어떤 작업을 원하시나요?\n"
}
db_Check(){
	script="select exists (select 1 from Information_schema.tables where table_name = 'sys' ) as flag;"
	mysql_execute "$script"
	#hello=$(mysql_execute "$script")
	#echo "$hello"
}
mysql_execute(){
	#sudo apt install mysql-client-5.7
        #sudo apt install mysql-client-core-5.7
        #mysql_config_editor set --login-path=$username --host=$mysqlDBip --user=$username --password (먼저 콘솔창에서 입력후 실행확인 필요)
	mysql --login-path=$username -e "$1"
	#return $?
}
menu_Select(){
	echo "값을 입력해주세요 : "
